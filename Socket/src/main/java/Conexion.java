//https://www.programarya.com/Cursos-Avanzados/Java/Sockets

import java.io.DataOutputStream;
import java.io.IOException;
import java.net.ServerSocket;
import java.net.Socket;

public class Conexion {

    private final int PUERTO = 1234; //Puerto para la conexión
    private final String HOST = "localhost"; //Host para la conexión
    protected String mensajeServidor; //Mensajes entrantes (recibidos) en el servidor
    protected ServerSocket serverSocket; //Socket del servidor
    protected Socket socketCliente  ; //Socket del cliente
    protected DataOutputStream salidaServidor, salidaCliente; //Flujo de datos de salida
    //Stream define que utiliza el protocoloTCP
    //Java siempre utiliza el dominio AF_INET para socket

    public Conexion(String tipo) throws IOException{
        //Constructor
        if(tipo.equalsIgnoreCase("servidor")){
            serverSocket = new ServerSocket(PUERTO);//Se crea el socket para el servidor en puerto 1234
            socketCliente = new Socket(); //Socket para el cliente
        }else{
            socketCliente = new Socket(HOST, PUERTO); //Socket para el cliente en localhost en puerto 1234
        }
    }
}
